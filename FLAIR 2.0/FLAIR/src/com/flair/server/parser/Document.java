...
class Document implements AbstractDocument
{
	...
	{
	    case ENGLISH:
		readabilityScoreCalc = Math.ceil(4.71 * ((double)numCharacters / (double)numTokens) + 0.5 * (numTokens / (double)numSentences) - 21.43);
		readabilityLevelThreshold_A = 7;
		readabilityLevelThreshold_B = 12;
		break;
	    case GERMAN:
		readabilityScoreCalc = Math.ceil(((double)numCharacters / (double)numTokens) + (numTokens / (double)numSentences));
