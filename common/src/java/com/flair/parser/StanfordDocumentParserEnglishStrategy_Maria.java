...
//                            pastPartFound = false;
    ...
    private boolean[] findIngPastPartBaseForTenses(TypedDependency dependency, IndexedWord verb, boolean[] found) {
        ...
        if (verb.tag().equalsIgnoreCase("vbg")) {
            ...
            addConstructionOccurrence(GrammaticalConstruction.TENSE_PRESENT_SIMPLE, verbBegin, verbEnd, dependency.toString());
            ingFound = false;
            pastPartFound = false;
            baseFormFound = false;
            ...
            addConstructionOccurrence(GrammaticalConstruction.TENSE_PAST_SIMPLE, verbBegin, verbEnd, dependency.toString());
            ingFound = false;
            pastPartFound = false;
            baseFormFound = false;
        }
    ...
    private void inspectQuestion(CoreLabel firstWord) {
        ...
        if (firstWordTag.toLowerCase().startsWith("w")) {
            ...
            addConstructionOccurrence(GrammaticalConstruction.QUESTIONS_YESNO, startInd, endInd, wordsOutput.toString());
            addConstructionOccurrence(GrammaticalConstruction.QUESTIONS_MODAL, startInd, endInd, wordsOutput.toString());
        } else {
    ...
    private void findTagQuestion(int endInd) {
        ...
        for (int i = wordsOutput.size() - 1; i > 0; i--) {
            ...
            } else if (lastWord != null && (!tag.equalsIgnoreCase(","))) // check for the negation or for the verb
            {
                ...
                if (count < 5) {
                    addConstructionOccurrence(GrammaticalConstruction.QUESTIONS_TAG, label.beginPosition(), endInd, wordsOutput.toString());  // highlight only the tag
                } else {
                    break;
                }
            } else {
        ...
        for (TypedDependency dependency : depsOutput) {
            ...
            if (rootFound
                ...
                if (dep.lemma().equalsIgnoreCase("be") || gov.lemma().equalsIgnoreCase("be")) {
                    ...
                    addConstructionOccurrence(GrammaticalConstruction.QUESTIONS_TO_BE, startInd, endInd, wordsOutput.toString());
                } else if (dep.lemma().equalsIgnoreCase("do") || gov.lemma().equalsIgnoreCase("do")) {
                    addWhQuestion(firstWordWord, startInd, endInd);
                    addConstructionOccurrence(GrammaticalConstruction.QUESTIONS_TO_DO, startInd, endInd, wordsOutput.toString());
                } else if (dep.lemma().equalsIgnoreCase("have") || gov.lemma().equalsIgnoreCase("have")) {
                    addWhQuestion(firstWordWord, startInd, endInd);
                    addConstructionOccurrence(GrammaticalConstruction.QUESTIONS_TO_HAVE, startInd, endInd, wordsOutput.toString());
                }
                ...
                case "had":
                    ...
                    if (wordsOutput.size() > thisInd + 1) {
                        if (wordsOutput.get(thisInd + 1).tag().equalsIgnoreCase("to")) {
                            ...
                            addConstructionOccurrence(GrammaticalConstruction.MODALS, label.beginPosition(), wordsOutput.get(thisInd + 1).endPosition(), labelWord + " to");
                            addConstructionOccurrence(GrammaticalConstruction.MODALS_ADVANCED, label.beginPosition(), wordsOutput.get(thisInd + 1).endPosition(), labelWord + " to");
                            addConstructionOccurrence(GrammaticalConstruction.MODALS_HAVE_TO, label.beginPosition(), wordsOutput.get(thisInd + 1).endPosition(), labelWord + " to");
                        }
                    }
    ...
    private void findPrepositions(TypedDependency dependency, int depBegin, int depEnd, String rel, IndexedWord dep, IndexedWord gov) {
        if (rel.contains("_")) {
    ...
    private void addSubjectiveAndObjectivePronouns(TypedDependency dependency, int start, int end, String rel, IndexedWord dep, int depBegin, int depEnd) {
