...
FLAIR.WEBRANKER.UTIL.resetSlider = function(name)
{
	if (name === "all")
	{
		$(".gradientSlider")
			.each(function()
			{
				...
				{
					if ($("#tgl-" + construct_name)
						.prop("checked") === true)
						$("#tgl-" + construct_name)
						.trigger("click");
				}
				else if ($("#tgl-" + construct_name)
					.prop("checked") === false)
					$("#tgl-" + construct_name)
					.trigger("click");
			});
	...
	{
		$("#collapse_" + name + " div[id$='-gradientSlider']")
			.each(function()
			{
				if ($(this)
					.slider("option", "value") !== 0)
...
{
	$("#exported_settings_url")
		.val(exportURL);
	$("#modal_ExportSettings")
		.modal('show');
};
...
FLAIR.WEBRANKER.UTIL.WAIT.INSTANCE = function()
{
	...
	var reset = function()
	{
		...
		$("#modal_waitIdle_buttonCancel")
			.hide();
		$("#modal_waitIdle_buttonYes")
			.hide();
		$("#modal_waitIdle_buttonNo")
			.hide();
...
{
	if (document.getElementById("show_all_constructions")
		.hidden)
	...
	document.getElementById("show_all_constructions")
		.hidden = (!document.getElementById("show_all_constructions")
			.hidden);
};
...
FLAIR.WEBRANKER.CUSTOMVOCAB = function(server_pipeline)
{
	...
	var apply = function()
	{
		...
		var tempList = $("#custom_vocab_textarea")
			.val();
		tempList = tempList.replaceAll("\n", ",");
		tempList = tempList.replaceAll(" ", ",");
		var wordArray = tempList.split(",");
		if (tempList.trim()
			.length < 2 || wordArray.length < 1)
		{
	...
	$("#modal_customVocab_buttonCancel")
		.click(function(e)
		{
			e.preventDefault();
			showDialog(false);
...
FLAIR.WEBRANKER.STATE = function()
{
	...
	var refreshWeightSettings = function()
	{
		...
		// doc levels
		var levels = document.getElementById("settings_levels")
		...
		// custom vocab
		var vocab = $("#customVocabList-gradientSlider")
			.slider("option", "value");
	...
	var getHighlightedDocHTML = function(doc)
	{
		...
		{
			var indexSt1 = (ind1["index"])
				.toString(); // String
			var indexSt2 = (ind2["index"])
				.toString(); // String
		...
		for (var ind in allIndices)
		{
			...
			var curItem = (allIndices[ind]["index"])
				.toString(); // String
			var tmpEnd = -1; // int
			var tmpStart = -1; // int
			// indexSt1 can be either of form start-end or just end
	...
	this.reset = function(full_reset)
	{
		...
		var label = $("#customVocabList-label")
			.text();
	...
	this.displayDocText = function(index)
	{
		...
		if (parsedDataFetched === true)
		{
			...
			if (previousSelection > -1 && document.getElementById("results_table")
				.childNodes.length > 0)
				$("#results_table tr:nth-child(" + (previousSelection + 1) + ")")
				...
				.childNodes.length > 0)
			{
				...
				$("#results_table tr:nth-child(" + (index + 1) + ")")
					.css("background-color", "#fdf6e6");
				...
				for (var i in weightSettings_constructions)
				{
					...
					for (var j in all)
					{
						if (all[j] === name)
						{
							...
							info_box_3 += "<tr><td>" + name_to_show + "</td><td class='text-cell'>" + doc["frequencies"][ind] + "</td><td class='text-cell'>(" + ((Number(doc["relFrequencies"][ind])) * 100)
								.toFixed(4) + ")</td></tr>";
						}
					}
				}
				...
				document.getElementById("snapshot")
					.innerHTML = info_box_1 + highlightedText + info_box_2 + info_box_3;

				var lines = document.getElementsByClassName("constructions_line");
				if (lines.length > 0)
				{
					document.getElementById("constructions-table")
						.hidden = false;
					if (lines.length === 1)
					{
						document.getElementById("info-highlights")
							.innerHTML = "";
						document.getElementById("info-highlights")
							.innerHTML = "<div class='panel panel-default' style='text-align: center'><a href='javascript:FLAIR.WEBRANKER.UTIL.TOGGLE.constructionsList();' style='color:darkgrey;'><span class='caret'></span> all constructions <span class='caret'></span></a></div><br>";
					}
					else
					{
						document.getElementById("info-highlights")
							.innerHTML = "* Highlights may overlap. Mouse over a highlight to see a tooltip<br> with the names of all embedded constructions.<br><br>" +
				...
				{
					document.getElementById("info-highlights")
						.innerHTML = "<button type='button' class='btn btn-warning' onclick='FLAIR.WEBRANKER.UTIL.TOGGLE.leftSidebar(true)' id='sidebar_grammar_button'>GRAMMAR SETTINGS</button>";
					document.getElementById("constructions-table")
						.hidden = true;
				}
			}
		...
		else
		{
				...
				.childNodes.length > 0)
			{
				...
				document.getElementById("snapshot")
					.innerHTML = info_box_1 + text_string;
	...
	this.rerank = function()
	{
		...
		for (var k = 0; k < parsedDocs.length; k++)
		{
			...
			for (var j = 0; j < weightSettings_constructions.length; j++)
			{
				if (weightSettings_constructions[j]["name"].indexOf("LEVEL") > -1)
				{
					...
					{
						$("#" + weightSettings_constructions[j]["name"])
							.prop('checked', false);
		...
		if (count !== 0)
			...
			weightSettings_customVocabList["idf"] = 0;
		...
		document.getElementById("docs_info")
			.innerHTML = (displayedDocs.length) + " results";
		for (var s in weightSettings_constructions)
		{
			if (weightSettings_constructions[s]["name"].startsWith("LEVEL"))
			{
				if (document.getElementById(weightSettings_constructions[s]["name"])
					.checked)
					document.getElementById(weightSettings_constructions[s]["name"] + "-df")
					.innerHTML = "(" + weightSettings_constructions[s]["df"] + " / " + displayedDocs.length + " results)";
				else
					document.getElementById(weightSettings_constructions[s]["name"] + "-df")
					.innerHTML = "";
			}
				...
				document.getElementById(weightSettings_constructions[s]["name"] + "-df")
				.innerHTML = "(" + weightSettings_constructions[s]["df"] + " / " + displayedDocs.length + ")";
		}
...
FLAIR.WEBRANKER.VISUALISATION = function(delegate_isDocFiltered, delegate_isConstructionEnabled)
{
	...
	this.visualise = function(csv_table)
	{
		...
		x.domain(dimensions = d3.keys(parsedArray[0])
			.filter(function(d)
			{
...
FLAIR.WEBRANKER.INSTANCE = function()
{
	...
	var queueSearchOp = function()
	{
		...
		state.setSearchParams(query, FLAIR.WEBRANKER.CONSTANTS.DEFAULT_LANGUAGE, document.getElementById("fetch_result_count")
			.value);
		state.flagAsBusy();
	...
	this.showVisualiser = function()
	{
		...
		if (state.isCustomCorpus() === false && state.getQuery() !== "")
			$("#query_vis")
			.html("\"" + state.getQuery() + "\" (" + state.getTotalResults() + " web pages)");
		else
			$("#query_vis")
...
window.onload = function()
{
	...
	$("#modal_CustomCorpus_buttonSelectFiles")
		.click(function(e)
		{
			e.preventDefault();
			$("#modal_CustomCorpus_fileSelect")
