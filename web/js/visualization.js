...
//    $("#graph_coordinates").html("");
    ...
    d3.csv(pathToConstructionDataCSV, function (error, docs) {
        ...
        x.domain(dimensions = d3.keys(docs[0]).filter(function (d) {
            return d !== "construction" && (is_in_settings(d) || is_selected(d) || d === "document" ||
                    d === "# of sentences" || d === "# of words" || d === "readability score")
                    && (y[d] = d3.scale.linear()
                            .domain(d3.extent(docs, function (p) {
                                return +p[d];
                            }))
                            .range([height, 0]));
        }));
