...
public class TestNPClarification extends TestCase {
	...
	public List<Tree> parseDoc(String doc) {
		...
		for (int i = 0; i < sents.size(); i++) {
			...
			Tree t = AnalysisUtilities.parseSentence(s).parse;
			res.add(t);
		}
	...
	public void testQuestionGeneration() {
		...
		/*for(Question q: res){
			ServerLogger.get().info("Q:"+q.getTree().yield().toString());
		}*/
		...
		//for(Question q: res){
		//	ServerLogger.get().trace(q.getFeatureValue("performedNPClarification")+"\t"+q.yield() );
		//}
