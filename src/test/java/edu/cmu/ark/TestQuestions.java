...
public class TestQuestions extends TestCase {
	...
	public void testComplementsInPPs() {
		Tree parse = AnalysisUtilities.parseSentence("Putin was president of Russia").parse;
		...
		parse = AnalysisUtilities.parseSentence("The president of Russia was Putin").parse;
		...
		parse = AnalysisUtilities.parseSentence("The capital of Alaska was visited by John.").parse;
		res = getQuestionOutputStringsFromParse(qt, parse);
		...
		parse = AnalysisUtilities.parseSentence("James saw John in the hall of mirrors.").parse;
		res = getQuestionOutputStringsFromParse(qt, parse);
		...
		parse = AnalysisUtilities.parseSentence("John read a book about linguistics.").parse;
		res = getQuestionOutputStringsFromParse(qt, parse);
	...
	public void testAuxiliariesAndVerbDecomposition() {
		Tree parse = AnalysisUtilities.parseSentence("I walked.").parse;
		List<String> res = getQuestionOutputStringsFromParse(qt, parse);
		...
		parse = AnalysisUtilities.parseSentence("I did a bad thing.").parse;
		res = getQuestionOutputStringsFromParse(qt, parse);
	...
	public void testVerbalComplements() {
		Tree parse = AnalysisUtilities.parseSentence("John knows that James plays baseball.").parse;
		List<String> res = getQuestionOutputStringsFromParse(qt, parse);
		...
		parse = AnalysisUtilities.parseSentence("John will play when Mary comes.").parse;
		res = getQuestionOutputStringsFromParse(qt, parse);
		...
		parse = AnalysisUtilities.parseSentence("John will play what Mary chooses.").parse;
		res = getQuestionOutputStringsFromParse(qt, parse);
		...
		parse = AnalysisUtilities.parseSentence("John will play whatever Mary chooses.").parse;
		res = getQuestionOutputStringsFromParse(qt, parse);
		assertTrue(res.toString(), res.contains("What will John play?"));
		assertTrue(res.toString(), res.contains("Who will play whatever Mary chooses?"));

		parse = AnalysisUtilities.parseSentence("John will play even if Mary does not come.").parse;
		res = getQuestionOutputStringsFromParse(qt, parse);
		assertFalse(res.toString(), res.contains("What will John play?"));
		assertTrue(res.toString(), res.contains("Who will play even if Mary does not come?"));

		parse = AnalysisUtilities.parseSentence("John is hopeful that James will play.").parse;
		res = getQuestionOutputStringsFromParse(qt, parse);
		...
		parse = AnalysisUtilities.parseSentence("James knows how to win.").parse;
		res = getQuestionOutputStringsFromParse(qt, parse);
		assertTrue(res.toString(), res.contains("What does James know?"));
		assertTrue(res.toString(), res.contains("Who knows how to win?"));

		parse = AnalysisUtilities.parseSentence("James knows how much money the car is worth.").parse;
		res = getQuestionOutputStringsFromParse(qt, parse);
		assertTrue(res.toString(), res.contains("What does James know?"));
	...
	public void testMainVerbCopula() {
		Tree parse = AnalysisUtilities.parseSentence("He was president.").parse;
		List<String> res = getQuestionOutputStringsFromParse(qt, parse);
		...
		parse = AnalysisUtilities.parseSentence("My favorite activity is to run in the park.").parse;
		res = getQuestionOutputStringsFromParse(qt, parse);
		assertFalse(res.toString(), res.contains("What is my favorite activity to run in?"));

		parse = AnalysisUtilities.parseSentence("My favorite activity seems to be to run in the park.").parse;
		res = getQuestionOutputStringsFromParse(qt, parse);
	...
	public void testSeem() {
		...
		parse = AnalysisUtilities.parseSentence(sentence).parse;
		res = getQuestionOutputStringsFromParse(qt, parse);
	...
	public void testWonderWho() {
		Tree parse = AnalysisUtilities.parseSentence("I wonder who he likes.").parse;
		List<String> res = getQuestionOutputStringsFromParse(qt, parse);
	...
	public void testPassiveDitransitiveRecipientIsNotIsland() {
		Tree parse = AnalysisUtilities.parseSentence("I was given a book by her.").parse;
		List<String> res;
		res = getQuestionOutputStringsFromParse(qt, parse);
	...
	public void testWithTelescope() {
		Tree parse = AnalysisUtilities.parseSentence("He saw her with a telescope.").parse;
		List<String> res;
		res = getQuestionOutputStringsFromParse(qt, parse);
		assertTrue(res.toString(), (res.size() == 4));
	...
	/*public void testAvoidPronounsInComplementPhrases(){
		Tree parse = AnalysisUtilities.parseSentence("John says he will study.").parse;
		List<String> res;
		res = getQuestionOutputStringsFromParse(qt, parse);
	...
	public void testExistentialThere() {
		...
		Tree parse = AnalysisUtilities.parseSentence(sentence).parse;
		List<String> res;
		res = getQuestionOutputStringsFromParse(qt, parse);
	...
	public void testPuntOnTwoNPs() {
		Tree parse = AnalysisUtilities.parseSentence("He last week issued a report.").parse;
		List<String> res;
		res = getQuestionOutputStringsFromParse(qt, parse);
		assertTrue(res.toString(), (res.size() == 2));
	...
	public void testComplementsVersusAdjuncts() {
		...
		parse = AnalysisUtilities.parseSentence("He believed John saw a miracle.").parse;
		res = getQuestionOutputStringsFromParse(qt, parse);
		...
		parse = AnalysisUtilities.parseSentence("He believed that John saw a miracle.").parse;
		res = getQuestionOutputStringsFromParse(qt, parse);
	...
	public void testActiveDitransitiveRecipientIsIsland() {
		Tree parse = AnalysisUtilities.parseSentence("He gave her a book.").parse;
		List<String> res;
		res = getQuestionOutputStringsFromParse(qt, parse);
		//assertTrue(res.toString(),res.contains("What was interesting?"));
	...
	public void testParsing() {
		Tree res = AnalysisUtilities.parseSentence("I walked.").parse;
	...
	public void testDontTreatSententialComplementsAsIslands() {
		Tree parse = AnalysisUtilities.parseSentence("I told you that he left the book.").parse;
		List<String> res;
		res = getQuestionOutputStringsFromParse(qt, parse);
	...
	public void testSubjectOfSententialComplementsCanMoveWhenNoComplementizerPresent() {
		Tree parse = AnalysisUtilities.parseSentence("I knew John read the book.").parse;
		List<String> res;
		res = getQuestionOutputStringsFromParse(qt, parse);
		assertTrue(res.toString(), (res.size() == 5));
		//assertTrue(res.toString(),res.contains("What was interesting?"));
	...
	public void testSubjectOfSententialComplementsCantMoveWhenComplementizerPresent() {
		Tree parse = AnalysisUtilities.parseSentence("I knew that John read the book.").parse;
		List<String> res;
		res = getQuestionOutputStringsFromParse(qt, parse);
		assertTrue(res.toString(), (res.size() == 4));
		//assertTrue(res.toString(),res.contains("What was interesting?"));
	...
	public void testTreatRelativeClausesAsIslands() {
		Tree parse = AnalysisUtilities.parseSentence("I bought the book that he saw.").parse;
		List<String> res;
		res = getQuestionOutputStringsFromParse(qt, parse);
	...
	public void testObjectOfPreposition() {
		Tree parse = AnalysisUtilities.parseSentence("He ran in the park.").parse;
		List<String> res;
		res = getQuestionOutputStringsFromParse(qt, parse);
	...
	public void testSubjectIsIsland() {
		Tree parse = AnalysisUtilities.parseSentence("That he ran was surprising.").parse;
		List<String> res;
		res = getQuestionOutputStringsFromParse(qt, parse);
	...
	public void testFormatting() {
		...
		parse = AnalysisUtilities.parseSentence("That man bought a car.").parse;
		res = getQuestionOutputStringsFromParse(qt, parse);
		assertTrue(res.toString(), res.contains("Who bought a car?"));
		assertTrue(res.toString(), res.contains("What did that man buy?"));

		parse = AnalysisUtilities.parseSentence("I like cars.").parse;
		res = getQuestionOutputStringsFromParse(qt, parse);
		...
		parse = AnalysisUtilities.parseSentence("Rivers likes cars.").parse;
		res = getQuestionOutputStringsFromParse(qt, parse);
		assertTrue(res.toString(), res.contains("What does Rivers like?"));

		//capitalization test for proper nouns at the start of the sentence 
		parse = AnalysisUtilities.parseSentence("Supporters followed Castro.").parse;
		res = getQuestionOutputStringsFromParse(qt, parse);
		...
		parse = AnalysisUtilities.parseSentence("I won $500.").parse;
		res = getQuestionOutputStringsFromParse(qt, parse);
		assertTrue(res.toString(), res.contains("Who won $500?"));


		parse = AnalysisUtilities.parseSentence("John lives in Pittsburgh, PA.").parse;
		res = getQuestionOutputStringsFromParse(qt, parse);
	...
	public void testMoveLeadingModifiers() {
		...
		parse = AnalysisUtilities.parseSentence("However, quickly, John ran.").parse;
		res = getQuestionOutputStringsFromParse(qt, parse);
		...
		parse = AnalysisUtilities.parseSentence("Fewer in numbers , the Red Kangaroo occupies deserts.").parse;
		res = getQuestionOutputStringsFromParse(qt, parse);
		...
		parse = AnalysisUtilities.parseSentence("During school, John rarely studied.").parse;
		res = getQuestionOutputStringsFromParse(qt, parse);
		...
		parse = AnalysisUtilities.parseSentence("While in school, John rarely studied.").parse;
		res = getQuestionOutputStringsFromParse(qt, parse);
		...
		parse = AnalysisUtilities.parseSentence("John rarely studied.").parse;
		res = getQuestionOutputStringsFromParse(qt, parse);
		assertFalse(res.toString(), res.contains("Rarely who studied?"));
		assertTrue(res.toString(), res.contains("Who rarely studied?"));

		parse = AnalysisUtilities.parseSentence("Separately, John studies.").parse;
		res = getQuestionOutputStringsFromParse(qt, parse);
		...
		parse = AnalysisUtilities.parseSentence("If I had money, I would buy a car.").parse;
		res = getQuestionOutputStringsFromParse(qt, parse);
		assertTrue(res.toString(), res.contains("If I had money, what would I buy?"));

		parse = AnalysisUtilities.parseSentence("John, while running, saw James.").parse;
		res = getQuestionOutputStringsFromParse(qt, parse);
...
public void testDontTransformSentencesWithLeadingModifiers() {
	Tree parse = AnalysisUtilities.parseSentence("He likes her, I said.").parse;
	List<String> res;
	qt.generateQuestionsFromParse(parse);
	res = qt.getQuestions();
