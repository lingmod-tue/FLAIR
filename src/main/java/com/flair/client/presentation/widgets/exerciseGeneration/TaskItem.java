...
public class TaskItem extends LocalizedComposite {
	...
	private static TaskItem.TaskItemLocalizationBinder localeBinder = GWT
			.create(TaskItem.TaskItemLocalizationBinder.class);
	...
	public TaskItem(ExerciseGenerationWidget parent, String name, int taskId) {
		...
		settingsWidgets = new Widget[] { grpBrackets, chkBracketsLemma, chkBracketsDistractorLemma,
				...
				chkBracketsTense, chkBracketsProgressive, chkBracketsActiveSentence, grpDistractors,
				chkDistractorsOtherForm, chkDistractorsOtherVariant, chkDistractorsOtherPast, chkDistractorsOtherTense,
				chkDistractorsProgressive, chkDistractorsIncorrectForms, chkDistractorsWrongConditional,
				chkDistractorsWrongClause, chkDistractorsWrongSuffixUse, chkDistractorsWrongSuffix, grpPos, grpCompForm,
				grpForms, grpVerbPerson, grpVerbForms, grpSentenceTypes, grpTenses, lblTensesSentences,
				chkPresentSimple, chkFutureSimple, chkPresentProgressive, chkPastProgressive, chkFutureProgressive,
				chkFuturePerfect, chkPresentPerfectProg, chkPastPerfectProg, chkFuturePerfectProg, lblTensesWords,
				grpCondTypes, grpClauses, grpScope, grpPronouns, grpVerbSplitting, grpTargetWords, grpSentTypes,
				chkPastSimple, chkPresentPerfect, chkPastPerfect, chkScopeActive, chkWho, chkWhich, chkThat,
				chkOtherRelPron, chk3Pers, chkNot3Pers, chkAffirmativeSent, chkNegatedSent, chkQuestions, chkStatements,
				chkRegularVerbs, chkIrregularVerbs, chkscopeType1, chkscopeType2, chkPosAdj, chkPosAdv,
				chkFormComparatives, chkFormSuperlatives, chkFormSynthetic, chkFormAnalytic, grpInstructions, chkLemmas,
				chkTenses, chkProgressive, chkNTargets };

		controlsToReset = new Widget[] { chkPosAdj, chkPosAdv, chkFormComparatives, chkFormSuperlatives,
				chkFormSynthetic, chkFormAnalytic, chkscopeType1, chkscopeType2, rbtMainClause, chkScopeActive,
				rbt1Verb, chkPresentSimple, chkPastSimple, chkFutureSimple, chkPresentProgressive, chkPastProgressive,
				chkFutureProgressive, chkPresentPerfect, chkPastPerfect, chkFuturePerfect, chkPresentPerfectProg,
				chkPastPerfectProg, chkFuturePerfectProg, chkAffirmativeSent, chkNegatedSent, chkQuestions,
				chkStatements, chkRegularVerbs, chkIrregularVerbs, chk3Pers, chkNot3Pers, chkWho, chkWhich, chkThat,
				chkOtherRelPron, rbtPerSentence, chkBracketsLemma, chkBracketsDistractorLemma, chkBracketsPos,
				chkBracketsForm, chkBracketsConditional, chkBracketsWill, chkBracketsSentenceType, chkBracketsTense,
				chkBracketsActiveSentence, chkBracketsProgressive, chkDistractorsOtherForm, chkDistractorsOtherVariant,
				chkDistractorsOtherPast, chkDistractorsOtherTense, chkDistractorsProgressive,
				chkDistractorsIncorrectForms, chkDistractorsWrongConditional, chkDistractorsWrongClause,
				chkDistractorsWrongSuffixUse, chkDistractorsWrongSuffix };
	 ...
	 */
	private final ArrayList<Pair<MaterialCheckBox, String>> distractorOptions = new ArrayList<>();

	/**
	 * Checkboxes representing possible brackets options
	 */
	private final ArrayList<Pair<MaterialCheckBox, String>> bracketsOptions = new ArrayList<>();

	/**
	 * Checkboxes representing possible instructions options
	 */
	private final ArrayList<Pair<MaterialCheckBox, String>> instructionsOptions = new ArrayList<>();

	/**
	 * Checkboxes representing possible outputFormats
	 */
	private final ArrayList<Pair<MaterialCheckBox, String>> formatOptions = new ArrayList<>();

	/**
	 * Checkboxes and radiobuttons which need to be set to <code>true</code> when
	 * resetting the panel
	 */
	final Widget[] controlsToReset;

	/**
	 * The occurrences of constructions relevant to exercise generation in the
	 * current document.
	 */
	...
	private class TargetConstruction {
		...
		public TargetConstruction(int startIndex, int endIndex, String constructionType) {
	 ...
	 * @return <code>true</code> if at least 2 of the checkboxes are visible and
	 *         checked; otherwise <code>false</code>
	 */
	...
	/**
	 * Generates the radiobuttons. We cannot specify them in xml since the
	 * radiogroup names must be unique over all tasks.
	 */
	...
	/**
	 * Determines all removed parts including the currently removed ones which are
	 * not contained in another removed part.
	 * 
	...
	private void initHandlers() {
		...
		txtDocumentForSelection.addKeyDownHandler(event -> {
		...
		for (Pair<MaterialCheckBox, String> option : distractorOptions) {
	...
	private void removeSelectionEventHanlder() {
		...
		lblDocumentForSelection
				.setText(lblDocumentForSelection.getText().substring(0, lblDocumentForSelection.getCursorPos())
						+ lblDocumentForSelection.getText().substring(
								lblDocumentForSelection.getCursorPos() + lblDocumentForSelection.getSelectionLength()));
	 ...
	 * Hides the brackets option for sentence type if only passive sentence are used
	 * and displays the Active sentence brackets option.
	 */
	private void onActiveSentenceClickedEventHandler(boolean recalculateNumberExercises) {
		if (recalculateNumberExercises) {
			setNumberExercisesText(calculateNumberOfExercises());
		}
		chkBracketsSentenceType.setVisible(getExerciseType().equals(ExerciseType.FILL_IN_THE_BLANKS)
				&& checkAtLeast2Checked(new MaterialCheckBox[] { chkScopeActive, chkScopePassive }));
		chkBracketsActiveSentence
				.setVisible(grpSentTypes.isVisible() && (!chkScopeActive.isVisible() || !chkScopeActive.getValue()));
	}

	/**
	 * Event handler method for when a tense checkbox has been clicked. Hides the
	 * brackets option for tense if less than 2 relevant tenses are checked.
	 */
	private void onTenseClickedEventHandler(boolean recalculateNumberExercises) {
		if (recalculateNumberExercises) {
			setNumberExercisesText(calculateNumberOfExercises());
		}
		chkBracketsTense.setVisible(getExerciseType().equals(ExerciseType.FILL_IN_THE_BLANKS) && checkAtLeast2Checked(
				new MaterialCheckBox[] { chkPresentSimple, chkPastSimple, chkFutureSimple, chkPresentProgressive,
						chkPastProgressive, chkFutureProgressive, chkPresentPerfect, chkPastPerfect, chkFuturePerfect,
						chkPresentPerfectProg, chkPastPerfectProg, chkFuturePerfectProg }));
	}

	/**
	 * Event handler method for when a POS checkbox has been clicked. Hides the
	 * brackets option for POS if less than 2 relevant tenses are checked.
	 */
	private void onPosClickedEventHandler(boolean recalculateNumberExercises) {
		if (recalculateNumberExercises) {
			setNumberExercisesText(calculateNumberOfExercises());
		}
		chkBracketsPos.setVisible(getExerciseType().equals(ExerciseType.FILL_IN_THE_BLANKS)
				&& checkAtLeast2Checked(new MaterialCheckBox[] { chkPosAdj, chkPosAdv }));
	}

	/**
	 * Event handler method for when a comparison form checkbox has been clicked.
	 * Hides the brackets and distractor options for comparison form if less than 2
	 * relevant tenses are checked.
	 */
	private void onComparisonFormClickedEventHandler(boolean recalculateNumberExercises) {
		if (recalculateNumberExercises) {
			setNumberExercisesText(calculateNumberOfExercises());
		}
		chkBracketsForm.setVisible(getExerciseType().equals(ExerciseType.FILL_IN_THE_BLANKS)
				&& checkAtLeast2Checked(new MaterialCheckBox[] { chkFormComparatives, chkFormSuperlatives }));
		chkDistractorsOtherForm.setVisible(getExerciseType().equals(ExerciseType.SINGLE_CHOICE)
				&& checkAtLeast2Checked(new MaterialCheckBox[] { chkFormComparatives, chkFormSuperlatives }));
	}

	/**
	 * Event handler method for when a form checkbox has been clicked. Hides the
	 * brackets and distractor options for variant form if less than 2 relevant
	 * tenses are checked.
	 */
	private void onFormClickedEventHandler(boolean recalculateNumberExercises) {
		...
		chkDistractorsOtherVariant.setVisible(getExerciseType().equals(ExerciseType.SINGLE_CHOICE)
				&& checkAtLeast2Checked(new MaterialCheckBox[] { chkFormAnalytic, chkFormSynthetic }));
	}

	/**
	 * Event handler method for when a conditional type checkbox has been clicked.
	 * Hides the brackets and distractor options for conditional type if less than 2
	 * relevant tenses are checked.
	 */
	private void onConditionalTypeClickedEventHandler(boolean recalculateNumberExercises) {
		...
		chkBracketsConditional.setVisible(getExerciseType().equals(ExerciseType.FILL_IN_THE_BLANKS)
				&& checkAtLeast2Checked(new MaterialCheckBox[] { chkscopeType1, chkscopeType2 }));
		chkDistractorsWrongConditional.setVisible(getExerciseType().equals(ExerciseType.SINGLE_CHOICE)
				&& checkAtLeast2Checked(new MaterialCheckBox[] { chkscopeType1, chkscopeType2 }));
	}
	...
	/**
	 * Sets the visibility of the settings parameters. Only those widgets given in
	 * the visible settings list are displayed.
	 * 
	 ...
	 * 
	 * @return The value of the currently selected topic, if any; otherwise the
	 *         empty string
	 */
	 ...
	 * 
	 * @return The value of the currently selected exercise type, if any; otherwise
	 *         the empty string
	 */
	...
	/**
	 * Shows only those exercise parameters that are relevant for the topic and
	 * exercise type and hides all others
	 */
	...
	/**
	 * Re-calculates the constructions relevant to exercise generation for the
	 * entire document and for the selected document part. Sets the text for
	 * document part selection to the previewed text and the selected range to the
	 * entire document. Sets the possible topics options in the dropdown.
	 */
	public void initializeRelevantConstructions() {
		// We only use the document if it's in the selected language
		if (!DocumentPreviewPane.getInstance().getCurrentlyPreviewedDocument().getDocument().getLanguage()
				.equals(DocumentPreviewPane.getInstance().getCurrentLocale())) {
			return;
		}
			...
			for (int n = 0; n < doc.getText().length(); n++) {
				...
				if (usedConstructions.size() > 0 && usedConstructions.get(0).getStartIndex() == n) {
					...
					if (usedConstructions.get(0).isToBeUsed()) {
						sb.append(
								"<button style='cursor:pointer;' class='selected' onclick='this.classList.toggle(\"selected\");'>");
					} else {
						sb.append(
								"<button style='cursor:pointer;' class='' onclick='this.classList.toggle(\"selected\");'>");
					}
				}
	...
	/**
	 * Determines the constructions which are targeted by the exercise according to
	...
	private ArrayList<String> determineConfiguredConstructions(boolean checkForValue) {
		...
		if (topic.equals(ExerciseTopic.PASSIVE)) {
			...
			if (exerciseType.equals(ExerciseType.FILL_IN_THE_BLANKS) || exerciseType.equals(ExerciseType.MARK_THE_WORDS)
					|| exerciseType.equals(ExerciseType.JUMBLED_SENTENCES)
					|| exerciseType.equals(ExerciseType.DRAG_AND_DROP) && rbtPerSentence.getValue()) {
				constructionsToConsider = getConstructionNamesFromSettings(
						constructionComponents.getRelativesFiBMarkComponents(), checkForValue);
			...
			if (exerciseType.equals(ExerciseType.FILL_IN_THE_BLANKS) || exerciseType.equals(ExerciseType.SINGLE_CHOICE)
					...
					|| exerciseType.equals(ExerciseType.JUMBLED_SENTENCES)) {
				constructionsToConsider = getConstructionNamesFromSettings(
						constructionComponents.getPresentFiBSelectComponents(), checkForValue);
			} else if (exerciseType.equals(ExerciseType.MARK_THE_WORDS)) {
				constructionsToConsider = getConstructionNamesFromSettings(
						constructionComponents.getPresentMarkComponents(), checkForValue);
			}
	...
	/**
	 * Calculates the maximum number of exercises that can be generated for the
	 * current document regardless of other settings configurations.
	 * 
	...
	/**
	 * Calculates the number of exercises that can be generated for the current
	 * document and settings and selections.
	 * 
	...
	public int calculateNumberOfExercises() {
		...
		for (String constructionToConsider : constructionsToConsider) {
			...
			for (TargetConstruction possibleConstruction : possibleConstructions) {
				...
				for (Pair<Integer, Integer> removedPart : removedParts) {
					if (Math.max(possibleConstruction.getStartIndex(), removedPart.first) < Math
							.min(possibleConstruction.getEndIndex(), removedPart.second)) {
		...
		for (TargetConstruction construction : containedConstructions) {
			for (TargetConstruction otherConstruction : containedConstructions) {
				if (construction != otherConstruction
						&& (Math.max(construction.getStartIndex(), otherConstruction.getStartIndex()) < Math
								.min(construction.getEndIndex(), otherConstruction.getEndIndex()))) {
					if (construction.getEndIndex() - construction.getStartIndex() > otherConstruction.getEndIndex()
							- otherConstruction.getStartIndex()) {
	 ...
	 * 
	 * @param constructionComponent A list of the individual constructions making up
	 *                              a more complex (more detailed) construction.
	 *                              Each construction consists of a list of possible
	 *                              values.
	 * @return The list of construction names corresponding to relevantConstructions
	 *         names
	 */
	private ArrayList<String> getConstructionNamesFromSettings(
			ArrayList<ArrayList<Pair<MaterialCheckBox, String>>> constructionLevels, boolean checkForValue) {
				...
				for (int i = 1; i < constructionLevels.size(); i++) {
					if (initialConstructionNames.size() > 0) {
						...
						for (Pair<MaterialCheckBox, String> currentLevelConstruction : constructionLevels.get(i)) {
							if (currentLevelConstruction.first == null || !checkForValue
									|| currentLevelConstruction.first.getValue()) {
								for (String initialConstructionName : initialConstructionNames) {
									currentConstructionNames
											.add(initialConstructionName + "-" + currentLevelConstruction.second);
								}
							}
						}
	 ...
	 * 
	 * @return <code>true</code> if at least 1 visible distractor option is checked;
	...
	/**
	 * Sets the text on whether and how many exercises can be generated according to
	 * the selected topic and exercise type, the individual parameter settings and
	...
	public void setNumberExercisesText(int numberOfExercises) {
		...
		if (chkUseConfig.isVisible() && chkUseConfig.getValue()) {
			...
			if (exerciseType.equals(ExerciseType.UNDEFINED) || topic.equals(ExerciseTopic.UNDEFINED)) {
				...
				if (numberOfExercises == 0) {
					...
					if (exerciseType.equals(ExerciseType.FILL_IN_THE_BLANKS)
						...
						lblNumberExercises.setText("A maximum of " + numberOfExercises
								+ " blanks can be generated for the current settings.");
					} else if (exerciseType.equals(ExerciseType.MARK_THE_WORDS)) {
						lblNumberExercises.setText("A maximum of " + numberOfExercises
								+ " target words can be generated for the current settings.");
					} else if (exerciseType.equals(ExerciseType.MEMORY)) {
						lblNumberExercises.setText("A maximum of " + numberOfExercises
								+ " pairs can be generated for the current settings.");
	...
	/**
	 * Filters those constructions that are within the selected part of the
	 * document.
	 * 
	...
	/**
	 * Calculates the occurrences of constructions in the combinations relevant to
	 * exercise generation.
	 */
	public void calculateConstructionsOccurrences(
			HashMap<String, ArrayList<TargetConstruction>> relevantConstructions) {
		...
		for (Entry<String, ArrayList<TargetConstruction>> entry : constructionOccurrences.entrySet()) {
			...
			for (TargetConstruction tc : entry.getValue()) {
				if (!excludedConstructions.stream().anyMatch(
						c -> c.getEndIndex() == tc.getEndIndex() && c.getStartIndex() == tc.getStartIndex())) {
	 ...
	 * @param group            The index (in terms of first, second, third or fourth
	 *                         part) of the construction in the name
	 * @return <code>true</code> if the construction occurs in the text; otherwise
	 *         <code>false</code>
	 */
	...
	/**
	 * Determines the names of all detailed constructions relevant to a topic with
	 * the value of one construction set.
	 * 
	 * @param topic The topic to which the construction belongs
	 * @param value The value of the construction
	 * @param group The index (in terms of first, second, third or fourth part) of
	 *              the construction in the name
	 * @return The list of detailed construction names to which the construction
	 *         belongs
	 */
	public ArrayList<String> getPartConstructionNames(String topic, String value, int group) {
		...
		// For the construction part that we are interested in, we only use those
		// constructions for our value.
		if (group == 1) {
	...
	public ExerciseSettings generateExerciseSettings() {
		...
		if (chkUseConfig.isVisible() && chkUseConfig.getValue()) {
			...
			ArrayList<String> distractorProperties = getSelectedDistractors();
			ArrayList<String> brackets = getSelectedBracketContents();
			ArrayList<String> instructions = getSelectedInstructionsContents();
			...
			if (type.equals(ExerciseType.DRAG_AND_DROP)) {
				if (topic.equals(ExerciseTopic.PASSIVE)) {
					type = ExerciseType.DRAG_AND_DROP_MULTI;
					if (rbt2Verbs.getValue()) {
						brackets.add(BracketsProperties.VERB_SPLITTING);
					}
				} else if (topic.equals(ExerciseTopic.PAST) || topic.equals(ExerciseTopic.COMPARISON)) {
					type = ExerciseType.DRAG_AND_DROP_SINGLE;
				} else {
			...
			} else if (topic.equals(ExerciseTopic.PASSIVE) && type.equals(ExerciseType.FILL_IN_THE_BLANKS)) {
				brackets.add(BracketsProperties.LEMMA);
			}

			return new DocumentExerciseSettings(constructions, doc.getUrl(), doc.getText(), removedParts, type, quiz,
					distractorProperties, brackets, instructions, spnNDistractors.getValue() - 1, lblName.getValue(),
					parent.chkDownloadResources.getValue(), chkOnlyText.getValue(),
					parent.chkGenerateFeedback.isVisible() && parent.chkGenerateFeedback.getValue(), doc.getLinkingId(),
					doc.getTitle(), "", outputFormats, topic);
		}
	}
	 ...
	 */
	private ArrayList<String> getSelectedDistractors() {
	 ...
	 */
	private ArrayList<String> getSelectedBracketContents() {
		ArrayList<String> brackets = new ArrayList<>();
		for (Pair<MaterialCheckBox, String> option : bracketsOptions) {
			if (option.first.isVisible() && option.first.getValue()) {
	 ...
	 */
	private ArrayList<String> getSelectedInstructionsContents() {
		ArrayList<String> instructions = new ArrayList<>();
		for (Pair<MaterialCheckBox, String> option : instructionsOptions) {
			if (option.first.isVisible() && option.first.getValue()) {
	 ...
	 * 
	 * @return The value of the currently selected quiz, if any; otherwise the empty
	 *         string
	 */
	public String getQuiz() {
		// We get a ClassCastException when we try to access the first list element, so
		// we just iterate over the (only) element
	 ...
	 * @param key                   The name of the construction used as key in the
	 *                              has map
	 */
	...
	/**
	 * Determines the indices of occurrences of constructions in the combinations
	 * relevant to exercise generation.
	 * 
	 * @return The indices of occurrences of constructions relevant to exercise
	 *         generation
	 */
	public HashMap<String, ArrayList<TargetConstruction>> getConstructionsOccurrences() {
		...
		// TODO: Maybe allow to specify in view whether to keep analytic forms as single
		// markable or as separate
		// we would have to multiply the occurrences of mark instances if this was
		// selected
		// for now, we keep them as one
		addSingleWordConstructions(relevantConstructions, GrammaticalConstruction.ADJECTIVE_COMPARATIVE_SHORT,
				DetailedConstruction.ADJ_COMP_SYN);
		addSingleWordConstructions(relevantConstructions, GrammaticalConstruction.ADJECTIVE_SUPERLATIVE_SHORT,
				DetailedConstruction.ADJ_SUP_SYN);
		addSingleWordConstructions(relevantConstructions, GrammaticalConstruction.ADJECTIVE_COMPARATIVE_LONG,
				DetailedConstruction.ADJ_COMP_ANA);
		addSingleWordConstructions(relevantConstructions, GrammaticalConstruction.ADJECTIVE_SUPERLATIVE_LONG,
				DetailedConstruction.ADJ_SUP_ANA);
		addSingleWordConstructions(relevantConstructions, GrammaticalConstruction.ADVERB_COMPARATIVE_SHORT,
				DetailedConstruction.ADV_COMP_SYN);
		addSingleWordConstructions(relevantConstructions, GrammaticalConstruction.ADVERB_SUPERLATIVE_SHORT,
				DetailedConstruction.ADV_SUP_SYN);
		addSingleWordConstructions(relevantConstructions, GrammaticalConstruction.ADVERB_COMPARATIVE_LONG,
				DetailedConstruction.ADV_COMP_ANA);
		addSingleWordConstructions(relevantConstructions, GrammaticalConstruction.ADVERB_SUPERLATIVE_LONG,
				DetailedConstruction.ADV_SUP_ANA);
		...
		GrammaticalConstruction[] tenseConstructions = new GrammaticalConstruction[] {
				GrammaticalConstruction.TENSE_PRESENT_SIMPLE, GrammaticalConstruction.TENSE_PAST_SIMPLE,
				GrammaticalConstruction.TENSE_FUTURE_SIMPLE, GrammaticalConstruction.TENSE_FUTURE_PERFECT,
				GrammaticalConstruction.TENSE_PRESENT_PROGRESSIVE,
				GrammaticalConstruction.TENSE_PRESENT_PERFECT_PROGRESSIVE,
				GrammaticalConstruction.TENSE_PAST_PROGRESSIVE, GrammaticalConstruction.TENSE_PAST_PERFECT_PROGRESSIVE,
				GrammaticalConstruction.TENSE_FUTURE_PROGRESSIVE,
				GrammaticalConstruction.TENSE_FUTURE_PERFECT_PROGRESSIVE, GrammaticalConstruction.TENSE_PRESENT_PERFECT,
				GrammaticalConstruction.TENSE_PAST_PERFECT, };

		ArrayList<TargetConstruction> passiveOccurrences = getConstructionsWithinSelectedPart(
				GrammaticalConstruction.PASSIVE_VOICE, doc);
		for (GrammaticalConstruction tenseConstruction : tenseConstructions) {
			ArrayList<TargetConstruction> tenseOccurrences = getConstructionsWithinSelectedPart(tenseConstruction, doc);
			...
			for (TargetConstruction tenseOccurrence : tenseOccurrences) {
				...
				for (TargetConstruction passiveOccurrence : passiveOccurrences) {
					if (passiveOccurrence.getStartIndex() >= tenseOccurrence.getStartIndex()
							&& passiveOccurrence.getStartIndex() < tenseOccurrence.getEndIndex()
							|| tenseOccurrence.getStartIndex() >= passiveOccurrence.getStartIndex()
									&& tenseOccurrence.getStartIndex() < passiveOccurrence.getEndIndex()) {
						// There is some overlap between the passive construction and the tense
						// construction
				...
				if (correspondingPassiveOccurrence != null) {
					// The passive construction doesn't always contain all verbs of a verb cluster
					// and the tense construction doesn't usually include the participle
					// We therefore take the combined construction
					passiveIndices.add(new TargetConstruction(
							Integer.min(correspondingPassiveOccurrence.getStartIndex(),
									tenseOccurrence.getStartIndex()),
		...
		tenseConstructions = new GrammaticalConstruction[] { GrammaticalConstruction.TENSE_PAST_SIMPLE,
				GrammaticalConstruction.TENSE_PRESENT_PERFECT, GrammaticalConstruction.TENSE_PAST_PERFECT,
				GrammaticalConstruction.TENSE_PAST_PROGRESSIVE, GrammaticalConstruction.TENSE_PAST_PERFECT_PROGRESSIVE,
				GrammaticalConstruction.TENSE_PRESENT_PERFECT_PROGRESSIVE };

		ArrayList<TargetConstruction> negationOccurrences = getConstructionsWithinSelectedPart(
				GrammaticalConstruction.NEGATION_NOT, doc);
		negationOccurrences.addAll(getConstructionsWithinSelectedPart(GrammaticalConstruction.NEGATION_NT, doc));
		ArrayList<TargetConstruction> questionOccurrences = getConstructionsWithinSelectedPart(
				GrammaticalConstruction.QUESTIONS_DIRECT, doc);
		ArrayList<TargetConstruction> irregularOccurrences = getConstructionsWithinSelectedPart(
				GrammaticalConstruction.VERBS_IRREGULAR, doc);
		...
		for (GrammaticalConstruction tenseConstruction : tenseConstructions) {
			...
			for (TargetConstruction tenseOccurrence : tenseOccurrences) {
				...
				for (TargetConstruction negationOccurrence : negationOccurrences) {
					if (negationOccurrence.getEndIndex() >= tenseOccurrence.getStartIndex() - 1
							&& negationOccurrence.getStartIndex() <= tenseOccurrence.getEndIndex() + 1) {
						// The negation is at most the previous or succeeding token (only a single
						// character in-between for a whitespace)
				...
				for (TargetConstruction questionOccurrence : questionOccurrences) {
					if (tenseOccurrence.getStartIndex() >= questionOccurrence.getStartIndex()
							&& tenseOccurrence.getEndIndex() <= questionOccurrence.getEndIndex()) {
				...
				for (TargetConstruction irregularOccurrence : irregularOccurrences) {
					if (irregularOccurrence.getStartIndex() >= tenseOccurrence.getStartIndex()
							&& irregularOccurrence.getEndIndex() <= tenseOccurrence.getEndIndex()) {
						// The irregular verb is part of the verb (irregular forms consist of a single
						// token)
				...
				if (foundNegationOverlap) {
					if (foundQuestionOverlap) {
						if (foundIrregularOverlap) {
							indicesQuestionNegativeIrregular.add(new TargetConstruction(
									Math.min(tenseOccurrence.getStartIndex(), negation.getStartIndex()),
									Math.max(tenseOccurrence.getEndIndex(), negation.getEndIndex()),
									tenseConstruction.name() + "-question-neg-irreg"));
						} else {
							indicesQuestionNegativeRegular.add(new TargetConstruction(
									Math.min(tenseOccurrence.getStartIndex(), negation.getStartIndex()),
									Math.max(tenseOccurrence.getEndIndex(), negation.getEndIndex()),
									tenseConstruction.name() + "-question-neg-reg"));
						}
					} else {
						if (foundIrregularOverlap) {
							indicesStatementNegativeIrregular.add(new TargetConstruction(
									Math.min(tenseOccurrence.getStartIndex(), negation.getStartIndex()),
									Math.max(tenseOccurrence.getEndIndex(), negation.getEndIndex()),
									tenseConstruction.name() + "-stmt-neg-irreg"));
						} else {
							indicesStatementNegativeRegular.add(new TargetConstruction(
									Math.min(tenseOccurrence.getStartIndex(), negation.getStartIndex()),
									Math.max(tenseOccurrence.getEndIndex(), negation.getEndIndex()),
									tenseConstruction.name() + "-stmt-neg-reg"));
						}
					}
				} else {
					if (foundQuestionOverlap) {
						...
						if (foundIrregularOverlap) {
							...
							indicesStatementAffirmativeRegular.add(new TargetConstruction(
									tenseOccurrence.getStartIndex(), tenseOccurrence.getEndIndex(),
									tenseConstruction.name() + "-stmt-affirm-reg"));
						}
					}
				}
			...
			relevantConstructions.put(tenseConstruction.name() + "-question-neg-irreg",
					indicesQuestionNegativeIrregular);
			...
			relevantConstructions.put(tenseConstruction.name() + "-question-affirm-irreg",
					indicesQuestionAffirmativeIrregular);
			relevantConstructions.put(tenseConstruction.name() + "-question-affirm-reg",
					indicesQuestionAffirmativeRegular);
			relevantConstructions.put(tenseConstruction.name() + "-stmt-affirm-irreg",
					indicesStatementAffirmativeIrregular);
			relevantConstructions.put(tenseConstruction.name() + "-stmt-affirm-reg",
					indicesStatementAffirmativeRegular);
		}

		// present tense combinations
		ArrayList<TargetConstruction> presentOccurrences = getConstructionsWithinSelectedPart(
				GrammaticalConstruction.TENSE_PRESENT_SIMPLE, doc);
		...
		for (TargetConstruction tenseOccurrence : presentOccurrences) {
			// we take the first token since the inflected present verb is usually at the
			// beginning of the sequence.
			// It's just an approximation, but the best we can get without proper NLP
			// processing.
			String occurrenceText = doc.getText()
					.substring(tenseOccurrence.getStartIndex(), tenseOccurrence.getEndIndex()).split(" ")[0];
			...
			boolean isThirdPersonSingular = occurrenceText != null
					&& (occurrenceText.endsWith("s") || occurrenceText.equals("doesn't")
							|| occurrenceText.equals("hasn't") || occurrenceText.equals("isn't"));

			if (foundNegationOverlap) {
				if (foundQuestionOverlap) {
					if (isThirdPersonSingular) {
						indicesQuestionNegative3.add(new TargetConstruction(
								Math.min(tenseOccurrence.getStartIndex(), negation.getStartIndex()),
								Math.max(tenseOccurrence.getEndIndex(), negation.getEndIndex()),
								DetailedConstruction.QUEST_NEG_3));
					} else {
						indicesQuestionNegativeNot3.add(new TargetConstruction(
								Math.min(tenseOccurrence.getStartIndex(), negation.getStartIndex()),
								Math.max(tenseOccurrence.getEndIndex(), negation.getEndIndex()),
								DetailedConstruction.QUEST_NEG_NOT3));
					}
				} else {
					if (isThirdPersonSingular) {
						indicesStatementNegative3.add(new TargetConstruction(
								Math.min(tenseOccurrence.getStartIndex(), negation.getStartIndex()),
								Math.max(tenseOccurrence.getEndIndex(), negation.getEndIndex()),
								DetailedConstruction.STMT_NEG_3));
					} else {
						indicesStatementNegativeNot3.add(new TargetConstruction(
								Math.min(tenseOccurrence.getStartIndex(), negation.getStartIndex()),
								Math.max(tenseOccurrence.getEndIndex(), negation.getEndIndex()),
								DetailedConstruction.STMT_NEG_NOT3));
					}
				}
			} else {
				if (foundQuestionOverlap) {
		...
		relevantConstructions.put(DetailedConstruction.QUEST_NEG_3, indicesQuestionNegative3);
		relevantConstructions.put(DetailedConstruction.QUEST_NEG_NOT3, indicesQuestionNegativeNot3);
		relevantConstructions.put(DetailedConstruction.STMT_NEG_3, indicesStatementNegative3);
		relevantConstructions.put(DetailedConstruction.STMT_NEG_NOT3, indicesStatementNegativeNot3);
		relevantConstructions.put(DetailedConstruction.QUEST_AFFIRM_3, indicesQuestionAffirmative3);
		relevantConstructions.put(DetailedConstruction.QUEST_AFFIRM_NOT3, indicesQuestionAffirmativeNot3);
		relevantConstructions.put(DetailedConstruction.STMT_AFFIRM_3, indicesStatementAffirmative3);
		relevantConstructions.put(DetailedConstruction.STMT_AFFIRM_NOT3, indicesStatementAffirmativeNot3);

		// relative pronouns
		ArrayList<TargetConstruction> relativeOccurrences = getConstructionsWithinSelectedPart(
				GrammaticalConstruction.PRONOUNS_RELATIVE, doc);
		...
		for (TargetConstruction relativeOccurrence : relativeOccurrences) {
			String occurrenceText = doc.getText().substring(relativeOccurrence.getStartIndex(),
					relativeOccurrence.getEndIndex());
			if (occurrenceText.equalsIgnoreCase("who")) {
				indicesWhoOccurrences.add(new TargetConstruction(relativeOccurrence.getStartIndex(),
						relativeOccurrence.getEndIndex(), DetailedConstruction.WHO));
			} else if (occurrenceText.equalsIgnoreCase("which")) {
				indicesWhichOccurrences.add(new TargetConstruction(relativeOccurrence.getStartIndex(),
						relativeOccurrence.getEndIndex(), DetailedConstruction.WHICH));
			} else if (occurrenceText.equalsIgnoreCase("that")) {
				indicesThatOccurrences.add(new TargetConstruction(relativeOccurrence.getStartIndex(),
						relativeOccurrence.getEndIndex(), DetailedConstruction.THAT));
			} else {
				indicesOtherOccurrences.add(new TargetConstruction(relativeOccurrence.getStartIndex(),
						relativeOccurrence.getEndIndex(), DetailedConstruction.OTHERPRN));
			}

		}
		relevantConstructions.put(DetailedConstruction.WHO, indicesWhoOccurrences);
		relevantConstructions.put(DetailedConstruction.WHICH, indicesWhichOccurrences);
		relevantConstructions.put(DetailedConstruction.THAT, indicesThatOccurrences);
		relevantConstructions.put(DetailedConstruction.OTHERPRN, indicesOtherOccurrences);
