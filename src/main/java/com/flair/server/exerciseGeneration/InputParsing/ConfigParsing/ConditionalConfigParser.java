...
public class ConditionalConfigParser {
	 ...
	 * @param ifClauseFirst        <c>true</c> if the if-clause is to be put before
	 *                             the main clause
	 * @param randomizeClauseOrder <c>true</c> if the order of if- and main clauses
	 ...
	 * 
	 * @param configData    The data from the config file
	 * @param ifClauseFirst <c>true</c> if the if-clause is to be put before the
	 *                      main clause
	 * @return The exercise information
	 */
	 ...
	 * 
	 * @param configData               The data from the config file
	 * @param ifClauseFirst            <c>true</c> if the if-clause is to be put
	 *                                 before the main clause
	 * @param targetIfClause           <c>true</c> if the constructions in the if
	 *                                 clause are to be targeted
	 * @param targetMainClause         <c>true</c> if the constructions in the main
	 *                                 clause are to be targeted
	 * @param nDistractors             The number of distractors to use for the
	 *                                 exercise
	 * @param randomizeClauseOrder     <c>true</c> if the order of if- and main
	 *                                 clauses is to be determined randomly for each
	 *                                 sentence.
	 * @param lemmasInBrackets         <c>true</c> if the lemma of the target is to
	 *                                 be given in brackets
	 * @param useDistractorLemma       <c>true</c> if a semantic distractor is to be
	 *                                 given in brackets
	 * @param targetEntireClause       <c>true</c> if the entire target clause is to
	 *                                 be converted into constructions
	 * @param giveLemmasInInstructions <c>true</c> if the lemmas of all targets and
	...
	private ExerciseData generateGapTask(ArrayList<ExerciseConfigData> configData, boolean ifClauseFirst,
			...
			boolean giveLemmasInInstructions) {
		ArrayList<TextPart> parts = new ArrayList<>();
		int sentenceId = 1;
		...
		for (ExerciseConfigData id : configData) {
				...
				if (targetAndClauseItems.getTargetPositions().size() == 0
					...
					if (inConstruction) {
						...
						if (nDistractors > 0) {
							...
							Collections.shuffle(targetAndClauseItems.getTargetDistractors());
			...
			if (inConstruction) {
				parts.add(new ConstructionTextPart(generateSentencesFromPositions(positionParts), sentenceId));
				positionParts.clear();
			}
	 ...
	 * @param ifClauseFirst        <c>true</c> if the if-clause is to be put before
	 *                             the main clause
	 * @param targetIfClause       <c>true</c> if the constructions in the if clause
	 *                             are to be targeted
	 * @param targetMainClause     <c>true</c> if the constructions in the main
	 *                             clause are to be targeted
	 * @param randomizeClauseOrder <c>true</c> if the order of if- and main clauses
	 ...
	 *                                FiB and SC exercises, Unterline for Underline
	 *                                exercises.
			...
			if (targetIfClause) {
				if (targetEntireClause) {
					...
					for (Pair<Integer, Integer> tp : constructionsIfClause) {
						targetPositions.add(
								new Pair<>(mainPositions.size() + 1 + tp.first, mainPositions.size() + 1 + tp.second));
					}
					targetDistractors.add(itemData.getDistractorsIfClause());
				}
				distractorLemmas.add(itemData.getDistractorLemmaIfClause());
	...
	public HashMap<String, ArrayList<HashMap<String, ExerciseData>>> parseConfigFile(InputStream inputStream) {
		...
		for (ExerciseConfigData ecd : configData) {
			...
			String key = cd.getActivity() + "_" + (cd.isType1VsType2() ? "condForm" : "condTypes");
		...
		for (Entry<String, ArrayList<ExerciseConfigData>> entry : configs.entrySet()) {
			...
			if (entry.getKey().endsWith("condForm")) {
				...
				// sc 1d
				exercises.put("" + 1,
						generateGapTask(entry.getValue(), true, true, false, 1, false, false, false, false, false));
				// sc 3d
				exercises.put("" + 2,
						generateGapTask(entry.getValue(), true, true, false, 3, false, false, false, false, false));
				// fib lemma blank
				exercises.put("" + 3,
						generateGapTask(entry.getValue(), true, true, false, 0, false, true, false, false, false));
				// fib lemma/distractor blank
				exercises.put("" + 4,
						generateGapTask(entry.getValue(), true, true, false, 0, false, true, true, false, false));
				// fib entire clause brackets
				exercises.put("" + 9,
						generateGapTask(entry.getValue(), true, true, false, 0, false, true, false, true, false));
				...
				// sc 1d
				exercises.put("" + 1,
						generateGapTask(entry.getValue(), false, true, false, 1, false, false, false, false, false));
				// sc 3d
				exercises.put("" + 2,
						generateGapTask(entry.getValue(), false, true, false, 3, false, false, false, false, false));
				// fib lemma blank
				exercises.put("" + 3,
						generateGapTask(entry.getValue(), false, true, false, 0, false, true, false, false, false));
				// fib lemma/distractor blank
				exercises.put("" + 4,
						generateGapTask(entry.getValue(), false, true, false, 0, false, true, true, false, false));
				// fib entire clause brackets
				exercises.put("" + 9,
						generateGapTask(entry.getValue(), false, true, false, 0, false, true, false, true, false));
				blocks.add(exercises);
				...
				// sc 1d
				exercises.put("" + 1,
						generateGapTask(entry.getValue(), false, true, false, 1, true, false, false, false, false));
				// sc 3d
				exercises.put("" + 2,
						generateGapTask(entry.getValue(), false, true, false, 3, true, false, false, false, false));
				// fib lemma blank
				exercises.put("" + 3,
						generateGapTask(entry.getValue(), false, true, false, 0, true, true, false, false, false));
				// fib lemma/distractor blank
				exercises.put("" + 4,
						generateGapTask(entry.getValue(), false, true, false, 0, true, true, true, false, false));
				// fib entire clause brackets
				exercises.put("" + 9,
						generateGapTask(entry.getValue(), false, true, false, 0, true, true, false, true, false));
				blocks.add(exercises);
				...
				// sc 1d
				exercises.put("" + 1,
						generateGapTask(entry.getValue(), true, false, true, 1, false, false, false, false, false));
				// sc 3d
				exercises.put("" + 2,
						generateGapTask(entry.getValue(), true, false, true, 3, false, false, false, false, false));
				// fib lemma blank
				exercises.put("" + 3,
						generateGapTask(entry.getValue(), true, false, true, 0, false, true, false, false, false));
				// fib lemma/distractor blank
				exercises.put("" + 4,
						generateGapTask(entry.getValue(), true, false, true, 0, false, true, true, false, false));
				// fib entire clause brackets
				exercises.put("" + 9,
						generateGapTask(entry.getValue(), true, false, true, 0, false, true, false, true, false));
				blocks.add(exercises);
				...
				// sc 1d
				exercises.put("" + 1,
						generateGapTask(entry.getValue(), false, false, true, 1, false, false, false, false, false));
				// sc 3d
				exercises.put("" + 2,
						generateGapTask(entry.getValue(), false, false, true, 3, false, false, false, false, false));
				// fib lemma blank
				exercises.put("" + 3,
						generateGapTask(entry.getValue(), false, false, true, 0, false, true, false, false, false));
				// fib lemma/distractor blank
				exercises.put("" + 4,
						generateGapTask(entry.getValue(), false, false, true, 0, false, true, true, false, false));
				// fib entire clause brackets
				exercises.put("" + 9,
						generateGapTask(entry.getValue(), false, false, true, 0, false, true, false, true, false));
				blocks.add(exercises);
				...
				// sc 1d
				exercises.put("" + 1,
						generateGapTask(entry.getValue(), false, false, true, 1, true, false, false, false, false));
				// sc 3d
				exercises.put("" + 2,
						generateGapTask(entry.getValue(), false, false, true, 3, true, false, false, false, false));
				// fib lemma blank
				exercises.put("" + 3,
						generateGapTask(entry.getValue(), false, false, true, 0, true, true, false, false, false));
				// fib lemma/distractor blank
				exercises.put("" + 4,
						generateGapTask(entry.getValue(), false, false, true, 0, true, true, true, false, false));
				// fib entire clause brackets
				exercises.put("" + 9,
						generateGapTask(entry.getValue(), false, false, true, 0, true, true, false, true, false));
				blocks.add(exercises);
				...
				// sc 1d
				exercises.put("" + 1,
						generateGapTask(entry.getValue(), true, true, true, 1, false, false, false, false, false));
				// sc 3d
				exercises.put("" + 2,
						generateGapTask(entry.getValue(), true, true, true, 3, false, false, false, false, false));
				// fib lemma blank
				exercises.put("" + 3,
						generateGapTask(entry.getValue(), true, true, true, 0, false, true, false, false, false));
				// fib lemma/distractor blank
				exercises.put("" + 4,
						generateGapTask(entry.getValue(), true, true, true, 0, false, true, true, false, false));
				// fib entire clause brackets
				exercises.put("" + 9,
						generateGapTask(entry.getValue(), true, true, true, 0, false, true, false, true, false));
				blocks.add(exercises);
				...
				// sc 1d
				exercises.put("" + 1,
						generateGapTask(entry.getValue(), false, true, true, 1, false, false, false, false, false));
				// sc 3d
				exercises.put("" + 2,
						generateGapTask(entry.getValue(), false, true, true, 3, false, false, false, false, false));
				// fib lemma blank
				exercises.put("" + 3,
						generateGapTask(entry.getValue(), false, true, true, 0, false, true, false, false, false));
				// fib lemma/distractor blank
				exercises.put("" + 4,
						generateGapTask(entry.getValue(), false, true, true, 0, false, true, true, false, false));
				// fib entire clause brackets
				exercises.put("" + 9,
						generateGapTask(entry.getValue(), false, true, true, 0, false, true, false, true, false));
				blocks.add(exercises);
				...
				// sc 1d
				exercises.put("" + 1,
						generateGapTask(entry.getValue(), false, true, true, 1, true, false, false, false, false));
				// sc 3d
				exercises.put("" + 2,
						generateGapTask(entry.getValue(), false, true, true, 3, true, false, false, false, false));
				// fib lemma blank
				exercises.put("" + 3,
						generateGapTask(entry.getValue(), false, true, true, 0, true, true, false, false, false));
				// fib lemma/distractor blank
				exercises.put("" + 4,
						generateGapTask(entry.getValue(), false, true, true, 0, true, true, true, false, false));
				// fib entire clause brackets
				exercises.put("" + 9,
						generateGapTask(entry.getValue(), false, true, true, 0, true, true, false, true, false));
				blocks.add(exercises);
			} else {
				exercises = new HashMap<>();
				// memory if-clause
				...
				exercises.put("" + 5,
						generateGapTask(entry.getValue(), true, true, false, 0, false, false, false, false, true));
				// jumbled sentences
				exercises.put("" + 6, generateJSTask(entry.getValue(), true, true, false, false));
				// underline
				exercises.put("" + 8,
						generateGapTask(entry.getValue(), true, true, false, 0, false, false, false, false, false));
				// fib entire clause brackets
				exercises.put("" + 9,
						generateGapTask(entry.getValue(), true, true, false, 0, false, true, false, true, false));
				blocks.add(exercises);
				...
				// fib lemma instuctions
				exercises.put("" + 5,
						generateGapTask(entry.getValue(), false, true, false, 0, false, false, false, false, true));
				// jumbled sentences
				exercises.put("" + 6, generateJSTask(entry.getValue(), false, true, false, false));
				// underline
				exercises.put("" + 8,
						generateGapTask(entry.getValue(), false, true, false, 0, false, false, false, false, false));
				// fib entire clause brackets
				exercises.put("" + 9,
						generateGapTask(entry.getValue(), false, true, false, 0, false, true, false, true, false));
				blocks.add(exercises);
				...
				// fib lemma instuctions
				exercises.put("" + 5,
						generateGapTask(entry.getValue(), false, true, false, 0, true, false, false, false, true));
				// jumbled sentences
				exercises.put("" + 6, generateJSTask(entry.getValue(), false, true, false, true));
				// underline
				exercises.put("" + 8,
						generateGapTask(entry.getValue(), false, true, false, 0, true, false, false, false, false));
				// fib entire clause brackets
				exercises.put("" + 9,
						generateGapTask(entry.getValue(), false, true, false, 0, true, true, false, true, false));
				blocks.add(exercises);
				...
				// fib lemma instuctions
				exercises.put("" + 5,
						generateGapTask(entry.getValue(), true, false, true, 0, false, false, false, false, true));
				// jumbled sentences
				exercises.put("" + 6, generateJSTask(entry.getValue(), true, false, true, false));
				// underline
				exercises.put("" + 8,
						generateGapTask(entry.getValue(), true, false, true, 0, false, false, false, false, false));
				// fib entire clause brackets
				exercises.put("" + 9,
						generateGapTask(entry.getValue(), true, false, true, 0, false, true, false, true, false));
				blocks.add(exercises);
				...
				// fib lemma instuctions
				exercises.put("" + 5,
						generateGapTask(entry.getValue(), false, false, true, 0, false, false, false, false, true));
				// jumbled sentences
				exercises.put("" + 6, generateJSTask(entry.getValue(), false, false, true, false));
				// underline
				exercises.put("" + 8,
						generateGapTask(entry.getValue(), false, false, true, 0, false, false, false, false, false));
				// fib entire clause brackets
				exercises.put("" + 9,
						generateGapTask(entry.getValue(), false, false, true, 0, false, true, false, true, false));
				blocks.add(exercises);
				...
				// fib lemma instuctions
				exercises.put("" + 5,
						generateGapTask(entry.getValue(), false, false, true, 0, true, false, false, false, true));
				// jumbled sentences
				exercises.put("" + 6, generateJSTask(entry.getValue(), false, false, true, true));
				// underline
				exercises.put("" + 8,
						generateGapTask(entry.getValue(), false, false, true, 0, true, false, false, false, false));
				// fib entire clause brackets
				exercises.put("" + 9,
						generateGapTask(entry.getValue(), false, false, true, 0, true, true, false, true, false));
				blocks.add(exercises);
