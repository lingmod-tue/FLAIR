...
public class QuestionFeatureExtractor {
	...
	private QuestionFeatureExtractor() {
		forwardLM = new LanguageModel(ResourceLoader.path("anc-v2-written.lm.gz"));
	}
	...
	private void extractVaguenessFeatures(Question q) {
		...
		if (Question.getFeatureNames().contains("numVagueNPsSource")) {
			specificityAnalyzer.analyze(sourceTree);
			extractCountAndGreaterThanFeatures(q, "numVagueNPsSource", 1, 5, specificityAnalyzer.getNumVagueNPs());
		}
		;

		if (Question.getFeatureNames().contains("numVagueNPsQuestion")) {
			specificityAnalyzer.analyze(tree);
			extractCountAndGreaterThanFeatures(q, "numVagueNPsQuestion", 1, 5, specificityAnalyzer.getNumVagueNPs());
		}

		if (answerPhraseTree != null) {
			if (Question.getFeatureNames().contains("numVagueNPsAnswer")) {
				specificityAnalyzer.analyze(answerPhraseTree);
				extractCountAndGreaterThanFeatures(q, "numVagueNPsAnswer", 1, 5, specificityAnalyzer.getNumVagueNPs());
			}
	...
	private void extractLangModelFeatures(Question q) {
		...
		double trigramSource = forwardLM.logBase10ProbabilityOfSentence(sourceTokens);
		double trigramQuestion = forwardLM.logBase10ProbabilityOfSentence(questionTokens);
		double meanUnigramSource = forwardLM.meanUnigramLogBase10Probability(sourceTokens);
		double meanUnigramQuestion = forwardLM.meanUnigramLogBase10Probability(questionTokens);
		...
		if (answerPhraseTree != null) {
			...
			double trigramAnswer = forwardLM.logBase10ProbabilityOfSentence(answerTokens);
			double meanUnigramAnswer = forwardLM.meanUnigramLogBase10Probability(answerTokens);
	...
	public static QuestionFeatureExtractor getInstance() {
		if (instance == null) {
			synchronized (QuestionFeatureExtractor.class) {
				if (instance == null)
